...
public class AILayer_HeroAssignation : AILayer, ITickable, ISimulationAIEvaluationHelper<UnitSkill>, IAIEvaluationHelper<UnitSkill, InterpreterContext>
{
	public AILayer_HeroAssignation()
	{
		...
		this.minimalUtilityBaseValue = 0.05f;
	...
	private float InfiltrationActionUtility_DamageFortification(InfiltrationAction action, AIData_Unit hero, SpiedGarrison spiedGarrison, City infiltratedCity, DebugScoring actionScoring)
	{
		...
		if (AILayer_War.IsWarTarget(base.AIEntity, infiltratedCity))
		{
	...
	{
		if (this.DiplomacyLayer.GetPeaceWish(base.AIEntity.Empire.Index))
		{
			return 0.3f;
		}
		float num = 0.1f;
	...
	private float ComputeGlobalHeroNeed()
	{
		...
		if (num4 > 400f + (float)this.departmentOfEducation.Heroes.Count * 250f && ((float)num3 < (float)this.departmentOfTheInterior.NonInfectedCities.Count - 1f || this.departmentOfEducation.Heroes.Count < 5))
		{
			return 1f;
		}
	...
	private float ComputeGovernorCityNeed()
	{
		...
		if (num < count && this.departmentOfEducation.Heroes.Count > 2 * count)
		{
			return 1f;
		}
	...
	private bool ExecuteChanges(AIData_Unit heroData)
	{
		...
		if (gameEntityGUID != y && gameEntityGUID.IsValid)
		{
			...
			{
				float num;
				if (!this.VisibleSpyGarrisons.Contains(gameEntityGUID) || !this.departmentOfIntelligence.CanInfiltrateIgnoreVision(heroData.Unit, heroData.HeroData.WantedHeroAssignation.Garrison, false, out num, true))
				{
	...
	private bool HeroLevelUp(AIData_Unit heroData)
	{
		if (heroData.Unit.GetPropertyValue(SimulationProperties.MaximumSkillPoints) - heroData.Unit.GetPropertyValue(SimulationProperties.SkillPointsSpent) > 0f)
		{
			...
			if (Amplitude.Unity.Framework.Application.Preferences.EnableModdingTools)
			{
				...
				{
					this.HeroSkillDecisions.Add(new KeyValuePair<string, string>(heroData.Unit.GUID.ToString(), decisionResult.ToString()));
				}
			}
	...
	private void OptimizeHeroAssignation()
	{
		...
		{
			if (assignationData is AssignationData_Spy && this.departmentOfIntelligence.IsGarrisonVisible(assignationData.Garrison))
			{
		...
		do
		{
			...
			for (int j = 0; j < this.heroAssignations.Count; j++)
			{
				...
				while (k < this.assignationData.Count)
				{
					...
					if ((this.VictoryLayer.NeedPreachers || this.VictoryLayer.NeedSettlers) && this.assignationData[k] is AssignationData_Army && this.assignationData[k].Garrison is Army)
					{
		...
		if (base.AIEntity.Empire.GetPropertyValue(SimulationProperties.NetEmpireMoney) > 50f)
		{
			...
			using (IEnumerator<Unit> enumerator = this.departmentOfEducation.Heroes.GetEnumerator())
			{
				...
				while (enumerator.MoveNext())
				{
					...
					if (unit != null && unit.Garrison == null && !DepartmentOfEducation.IsInjured(unit) && !DepartmentOfEducation.IsCaptured(unit))
					{
						AIData_Unit aidata_Unit;
						...
						if (aidata_Unit != null && aidata_Unit.HeroData != null)
						{
							...
							{
								if (aidata_Unit.HeroData.WantedHeroAssignation.Garrison.Empire == base.AIEntity.Empire)
								{
									continue;
								}
							}
	...
	public List<string> GetHeroSkillDecisions(AIData_Unit heroData)
	{
		...
		if (list.Count == 0)
		{
			...
			if (list2.Count > 0)
			{
				...
				foreach (DecisionResult decisionResult in this.decisionResults)
				{
					list.Add(decisionResult.ToString());
				}
			}
		}
		return list;
	...
	{
		if (base.AIEntity.Empire.SimulationObject.Tags.Contains("FactionTraitBuyOutPopulation"))
		{

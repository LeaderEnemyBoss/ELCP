...
{
	public AILayer_Trade()
	{
		this.affinitiesDecisionResults = new List<DecisionResult>();
		this.magicResourceSellingProportion = 0.5f;
		this.minimalMagicTimeRange = 4;
		this.maximalMagicTimeRange = 10;
		this.armiesThatTriedKaijuTameResourceTrading = new List<GameEntityGUID>();
	}
	...
	public override void Release()
	{
		...
		this.armiesThatTriedKaijuTameResourceTrading.Clear();
		base.Release();
	}
	...
	protected override void ExecuteNeeds(StaticString context, StaticString pass)
	{
		...
		if (this.departmentOfScience.CanTradeUnits(false))
		{
			...
			if (!base.AIEntity.Empire.GetAgency<DepartmentOfForeignAffairs>().IsInWarWithSomeone() && this.departmentOfScience.CanCreatePrivateers() && base.AIEntity.Empire.GetPropertyValue(SimulationProperties.NetEmpireMoney) > 100f && !this.SaveMoney)
			{
	...
	private SynchronousJobState SynchronousJob_ProcessPrivateerOrders()
	{
		...
		this.privateertick++;
		if (this.aILayer_ArmyManagement == null)
		{
			return SynchronousJobState.Failure;
		}
				...
				if (service2 != null && service2.TryGetValue(orderTransferGarrisonToNewArmy.ArmyGuid, out gameEntity))
				{
					...
					if (army != null)
					{
						...
						foreach (AICommander aicommander in this.aILayer_ArmyManagement.AICommanders)
						{
							if (aicommander is AICommander_WarWithObjective)
							{
								AICommanderMission aicommanderMission = aicommander.Missions.Find((AICommanderMission match) => match is AICommanderMission_PrivateersHarass && !match.AIDataArmyGUID.IsValid);
								if (aicommanderMission != null)
								{
									aicommanderMission.AIDataArmyGUID = army.GUID;
									aicommanderMission.Promote();
									break;
								}
							}
						}
	...
	private Army GetSmallMercArmy()
	{
		...
		for (int i = 0; i < agency.Armies.Count; i++)
		{
			...
			if (army.GUID.IsValid && army.StandardUnits.Count > 0 && army.StandardUnits.Count < (int)base.AIEntity.Empire.GetPropertyValue(SimulationProperties.ArmyUnitSlot) && !army.IsSeafaring && (aidata == null || !(aidata.CommanderMission is AICommanderMission_RegroupArmyAt)))
			{
	...
	private SynchronousJobState SynchronousJob_ProcessRichboyResourceTradingOrder()
	{
		...
		if (this.queuedRichboyResource == null)
		{
			...
			if (num < 1000f || (float)this.departmentOfEducation.Heroes.Count < (float)this.departmentOfTheInterior.Cities.Count * 1.5f || base.AIEntity.Empire.GetPropertyValue(SimulationProperties.NetEmpireMoney) < 100f || this.departmentOfScience.CurrentTechnologyEraNumber < 2)
			{
			...
			foreach (KeyValuePair<StaticString, float> keyValuePair2 in this.aILayer_QuestBTController.ResourcesNeededForQuest)
			{
				list.AddOnce(keyValuePair2.Key.ToString());
			}
			using (List<string>.Enumerator enumerator3 = list.GetEnumerator())
			{
				while (enumerator3.MoveNext())
				{
					...
					if (resourceDefinition.ResourceType == ResourceDefinition.Type.Luxury)
					{
						...
						else if (this.aILayer_QuestBTController.ResourcesNeededForQuest.TryGetValue(text, out num3) && num3 > 0f)
						{
							saveThis = true;
						}
	...
	public void ProcessKaijuTameResourceTradingOrder(string LuxuryName, float TameCost)
	{
		if (!this.departmentOfScience.CanTradeResourcesAndBoosters(false))
		{
			return;
		...
		float num2;
		if (!this.departmentOfTheTreasury.TryGetResourceStockValue(base.AIEntity.Empire.SimulationObject, LuxuryName, out num2, false))
		{
			return;
		}
		TameCost -= num2;
		...
		TameCost = Mathf.Ceil(TameCost);
		TradableResource tradableResource = this.TryGetTradableRessource(LuxuryName);
		if (tradableResource != null && tradableResource.Quantity >= TameCost)
		{
			float priceWithSalesTaxes = TradableResource.GetPriceWithSalesTaxes(LuxuryName, TradableTransactionType.Buyout, base.AIEntity.Empire, TameCost);
			float num3 = (TameCost <= 10f) ? 1.1f : (1.1f + (TameCost - 10f) / 20f);
			if (num >= priceWithSalesTaxes * num3)
			{
				while (TameCost > 0f)
				{
					float num4 = Mathf.Min(10f, TameCost);
					TameCost -= num4;
					float priceWithSalesTaxes2 = TradableResource.GetPriceWithSalesTaxes(LuxuryName, TradableTransactionType.Buyout, base.AIEntity.Empire, num4);
					num -= priceWithSalesTaxes2;
					if (num <= 0f)
					{
						return;
					}
					base.AIEntity.Empire.PlayerControllers.AI.PostOrder(new OrderBuyoutTradable(base.AIEntity.Empire.Index, tradableResource.UID, num4));
				}
			}
		}
	}

	public List<GameEntityGUID> ArmiesThatTriedKaijuTameResourceTrading
	{
		get
		{
			return this.armiesThatTriedKaijuTameResourceTrading;
		}
		set
		{
			this.armiesThatTriedKaijuTameResourceTrading = value;
		}
	}
	...
	private List<DecisionResult> affinitiesDecisionResults;
	...
	private float magicResourceSellingProportion;

	[InfluencedByPersonality]
	private int minimalMagicTimeRange;

	[InfluencedByPersonality]
	private int maximalMagicTimeRange;
	...
	private List<GameEntityGUID> armiesThatTriedKaijuTameResourceTrading;

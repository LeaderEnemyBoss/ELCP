...
public class AchievementManager : SteamAchievementManager
{
	...
	private void OnEventWorldTerraformed(Amplitude.Unity.Event.Event eventRaised)
	{
		...
		if (eventEmpireWorldTerraformed.Empire.Index != this.ActiveEmpire.Index || eventEmpireWorldTerraformed.Reversible)
		{
			return;
		}
		base.AddToStatistic("TILES_TERRAFORMED_OVERALL_COUNT", (float)eventEmpireWorldTerraformed.TerraformedTiles.Length, false);
		IWorldPositionningService service = this.gameService.Game.Services.GetService<IWorldPositionningService>();
		...
		for (int j = 0; j < list.Count; j++)
		{
			...
			if (flag)
			{
				base.SetAchievement("ENEMY_EXPLOITATION_TERRAFORMED");
				return;
			}
		}
	}

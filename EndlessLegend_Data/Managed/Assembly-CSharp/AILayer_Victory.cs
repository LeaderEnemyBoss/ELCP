...
public class AILayer_Victory : AILayerWithObjective, IXmlSerializable
{
	public AILayer_Victory() : base("Victory")
	{
		...
		this.currentFocus = string.Empty;
	}
	...
	{
		this.NeedPreachers = false;
		this.NeedSettlers = false;
		...
		if (this.ActiveVictoryQuest != string.Empty && this.globalObjectiveMessages.Count == 0)
		{
			...
			else if (this.ActiveVictoryQuest == "VictoryQuest-Chapter5" || this.ActiveVictoryQuest == "VictoryQuest-Chapter5Alt")
			{
		...
		foreach (GlobalObjectiveMessage globalObjectiveMessage in this.globalObjectiveMessages)
		{
			...
			float num = 1f;
			if (region.City != null && region.City.Empire.Index != base.AIEntity.Empire.Index && (this.departmentOfForeignAffairs.IsAtWarWith(region.City.Empire) || (!this.departmentOfForeignAffairs.IsFriend(region.City.Empire) && this.departmentOfForeignAffairs.IsInWarWithSomeone())))
			{
				num = base.AIEntity.Empire.GetPropertyValue(SimulationProperties.MilitaryPower) / region.City.Empire.GetPropertyValue(SimulationProperties.MilitaryPower);
		...
		if ((float)this.game.Turn >= this.EarliestVictoryEvaluationTurn && (this.currentFocus == string.Empty || this.game.Turn % 8 == base.AIEntity.Empire.Index) && base.AIEntity.Empire.SimulationObject.GetPropertyValue(SimulationProperties.CurrentEra) > 3f)
		{
	...
	private void TryGenerateChapter3Objective()
	{
		...
		foreach (QuestBehaviourTreeNode_Action_AddQuestMarker questBehaviourTreeNode_Action_AddQuestMarker in list)
		{
			...
			if (this.questManagementService.TryGetMarkerByGUID(questBehaviourTreeNode_Action_AddQuestMarker.QuestMarkerGUID, out questMarker) && questMarker.IsVisibleInFogOfWar)
			{
				...
				if (pointOfInterest != null && this.departmentOfForeignAffairs.CanMoveOn(pointOfInterest.Region.Index, false) && (pointOfInterest.Region.City == null || pointOfInterest.Region.City.Empire.Index == base.AIEntity.Empire.Index || this.departmentOfForeignAffairs.DiplomaticRelations[pointOfInterest.Region.City.Empire.Index].State.Name == DiplomaticRelationState.Names.War || this.POIAccessible(pointOfInterest)))
				{
		...
		if (list2.Count > 0)
		{
			List<PointOfInterest> list3 = list2.FindAll((PointOfInterest x) => x.Region != null && x.Region.City != null && x.Region.City.Empire.Index != base.AIEntity.Empire.Index);
			if (list3.Count == 0)
			{
				list3 = list2.FindAll((PointOfInterest x) => x.Region != null && x.Region.City == null);
			}
			if (list3.Count == 0)
			{
				list3 = list2;
	...
	public override void WriteXml(XmlWriter writer)
	{
		if (writer.WriteVersionAttribute(3) >= 3)
		{
			...
			writer.WriteAttributeString<string>("CurrentFocus", this.currentFocus);
		}
	...
	public override void ReadXml(XmlReader reader)
	{
		if (reader.ReadVersionAttribute() > 2)
		{
			...
			this.currentFocus = reader.GetAttribute<string>("CurrentFocus", string.Empty);
		}
		base.ReadXml(reader);
	}
	...
	{
		if (this.departmentOfTheInterior.Cities.Count == 0 || this.VictoryWonder == null || !DepartmentOfTheTreasury.CheckConstructiblePrerequisites(this.departmentOfTheInterior.Cities[0], this.VictoryWonder, new string[]
		{
		...
		{
			if (this.departmentOfIndustry.GetConstructionQueue(this.departmentOfTheInterior.Cities[i]).Contains((Construction x) => x.ConstructibleElement.SubCategory == "SubCategoryVictory"))
			{
				this.FocusVictoryWonder = false;
				this.TryingToBuildVictoryWonder = false;
				return;
		...
		List<City> list = this.departmentOfTheInterior.Cities.ToList<City>();
		list.Sort((City left, City right) => -1 * left.GetPropertyValue(SimulationProperties.NetCityProduction).CompareTo(right.GetPropertyValue(SimulationProperties.NetCityProduction)));
		int num = -1;
		for (int j = 0; j < list.Count; j++)
		{
			if (AILayer_Military.AreaIsSave(list[j].WorldPosition, 5, this.departmentOfForeignAffairs, false))
			{
				num = j;
				break;
			}
	...
	private void ComputeVictoryFocus()
	{
		...
		foreach (KeyValuePair<string, float> keyValuePair in orderedVictoryPreferences)
		{
			...
			{
				this.currentFocus = keyValuePair.Key;
				flag = true;
				break;
			}
		}
		if (!flag)
		{
			foreach (KeyValuePair<string, float> keyValuePair2 in orderedVictoryPreferences)
			{
				...
				{
					this.currentFocus = keyValuePair2.Key;
					flag = true;
					break;
				}
			}
	...
	private bool FocusMilitary(int Rank)
	{
		...
		if (this.currentFocus == ELCPUtilities.AIVictoryFocus.Military || this.currentFocus == string.Empty)
		{
	...
	private bool FocusDiplomacy(int Rank)
	{
		...
		if (this.currentFocus == ELCPUtilities.AIVictoryFocus.Diplomacy || this.currentFocus == string.Empty)
		{
	...
	private bool FocusEconomy(int Rank)
	{
		...
		num = AILayer.Boost(num, num2);
		num = AILayer.Boost(num, (float)Rank * 0.15f);
		if (this.currentFocus == ELCPUtilities.AIVictoryFocus.Economy || this.currentFocus == string.Empty)
		{
			num = AILayer.Boost(num, 0.15f);
		}
		if (Amplitude.Unity.Framework.Application.Preferences.EnableModdingTools)
	...
	private bool FocusMostTechnologiesDiscovered(int Rank)
	{
		...
		if (this.currentFocus == ELCPUtilities.AIVictoryFocus.Technology || this.currentFocus == string.Empty)
		{
	...
	private List<KeyValuePair<string, float>> GetOrderedVictoryPreferences()
	{
		...
		{
			if (pair1.Key == this.currentFocus)
			{
				return -1;
			}
			if (pair2.Key == this.currentFocus)
			{
				return 1;
			}

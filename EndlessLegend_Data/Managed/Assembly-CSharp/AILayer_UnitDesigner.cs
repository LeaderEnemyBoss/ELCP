...
public class AILayer_UnitDesigner : AILayer, ISimulationAIEvaluationHelper<AIItemData>, IAIEvaluationHelper<AIItemData, InterpreterContext>
{
	...
	{
		bool flag = !this.CurrentUnitDesign.Tags.Contains(DownloadableContent9.TagColossus);
		float num;
		...
		float num2 = 0f;
		bool flag2 = this.IgnoreCostForCurrentDesign;
		...
		if (!flag2 && this.Hero != null && this.HeroDesign && this.unitDesignRepository.TryGetUnitDesignData(base.AIEntity.Empire.Index, this.Hero.UnitDesign.Model, out aiunitDesignData) && aiunitDesignData.OldUnitDesignScoring.ItemNamePerSlot.Contains(element.ToString()))
		{
			flag2 = true;
		}
		if (!flag2)
		{
			for (int i = 0; i < element.ItemDefinition.Costs.Length; i++)
			{
				...
				for (int j = 0; j < this.resourcePolicy.Count; j++)
				{
					...
					{
						float value = element.ItemDefinition.Costs[i].GetValue(base.AIEntity.Empire);
						float num3;
						float num4;
						if (element.ItemDefinition.Costs[i].ResourceName == "EmpireMoney" && num > 200f)
						{
							num3 = this.resourcePolicy[j].Value + num / 50f;
						}
						...
						{
							num3 = this.resourcePolicy[j].Value;
							if (this.Hero == null && element.ItemDefinition.Costs[i].ResourceName.ToString().Contains("Strategic") && this.departmentOfTheTreasury.TryGetResourceStockValue(base.AIEntity.Empire.SimulationObject, element.ItemDefinition.Costs[i].ResourceName, out num4, false))
							{
								if (flag && this.ExpectedResourceUsage[element.ItemDefinition.Costs[i].ResourceName.ToString()] <= 0f)
								{
									return -2f;
								}
								if (flag && this.OriginalNonHeroDesign)
								{
									Dictionary<string, float> expectedResourceUsage = this.ExpectedResourceUsage;
									string key = element.ItemDefinition.Costs[i].ResourceName.ToString();
									expectedResourceUsage[key] -= element.ItemDefinition.Costs[i].GetValue(base.AIEntity.Empire) * this.ResourceLimitMultiplier;
								}
								if (!element.IsNaval && num4 > 80f && num4 / 2f > this.resourcePolicy[j].Value && this.CurrentUnitDesign.UnitBodyDefinition.SubCategory != "SubCategorySupport")
								{
									num3 = num4 / 2f;
								}
							}
						}
						num2 = Mathf.Min(1f, Mathf.Max(num2, value / num3));
						flag3 = true;
						break;
					}
				}
		...
		if (this.Hero != null && this.HeroDesign)
		{
			...
			{
				if (!flag4 && (element.AIParameters[k].Name.ToString().Contains("Governor") || element.AIParameters[k].Name.ToString().Contains("AIItem")))
				{
					flag4 = (element.AIParameters[k].GetValue(new InterpreterContext(this.Hero.SimulationObject)) > 0f);
				}
				if (!flag5 && (element.AIParameters[k].Name.ToString().Contains("AIEquipment") || element.AIParameters[k].Name.ToString().Contains("AIUnit") || element.AIParameters[k].Name.ToString().Contains("Army")))
				{
					flag5 = (element.AIParameters[k].GetValue(new InterpreterContext(this.Hero.SimulationObject)) > 0f);
				}
				if (!flag6 && element.AIParameters[k].Name.ToString().Contains("Spy"))
				{
					flag6 = (element.AIParameters[k].GetValue(new InterpreterContext(this.Hero.SimulationObject)) > 0f);
				}
			}
			...
			if (!this.Governorhero && !this.Spyhero && currentScore > 0f && !flag5)
			{
				currentScore = 0f;
			}
			if (this.Spyhero)
		...
		if (!this.HeroDesign && currentScore <= this.scoreLimit && element.ResourceName == "Iron" && this.bestTierByResourceName[element.ResourceName] == element.Tier && !this.CurrentUnitDesign.CheckUnitAbility(UnitAbility.ReadonlyColonize, -1))
		{
			currentScore = this.scoreLimit + 0.01f;
		}
		return currentScore;
	}
	...
	protected override void CreateLocalNeeds(StaticString context, StaticString pass)
	{
		...
		this.ResourceLimitMultiplier = Mathf.Min(3f, Mathf.Max(1f, base.AIEntity.Empire.GetPropertyValue(SimulationProperties.EmpireScaleFactor) / 5f));
		this.empireScaleLimit = Mathf.Max(2.5f, base.AIEntity.Empire.GetPropertyValue(SimulationProperties.EmpireScaleFactor) * 0.5f);
		for (int j = 0; j < this.departmentOfEducation.Heroes.Count; j++)
		{
			if (Amplitude.Unity.Framework.Application.Preferences.EnableModdingTools)
			{
				...
				{
					Diagnostics.LogError("ELCP {0} GenerateHeroRetrofitNeeds failed for {1}", new object[]
					{
						base.AIEntity.Empire,
						this.departmentOfEducation.Heroes[j].UnitDesign.LocalizedName
					});
				}
			}
			else
		...
		list2.Sort(delegate(UnitDesign left, UnitDesign right)
		{
			bool flag = left.Tags.Contains(DownloadableContent9.TagColossus);
			bool flag2 = right.Tags.Contains(DownloadableContent9.TagColossus);
			bool flag3 = left.UnitBodyDefinition.SubCategory == "SubCategorySupport";
			bool flag4 = right.UnitBodyDefinition.SubCategory == "SubCategorySupport";
			bool flag5 = right.Tags.Contains(DownloadableContent16.TagSeafaring);
			bool flag6 = left.Tags.Contains(DownloadableContent16.TagSeafaring);
			if (flag && !flag2)
			{
				return -1;
			}
	...
	private void GenerateHeroRetrofitNeeds(Unit hero)
	{
		...
		if (hero == null || hero.UnitDesign == null)
		{
			return;
		}
		...
		if (hero.Garrison != null)
		{
			...
			if (hero.Garrison is City)
			{
				...
				using (IEnumerator<SimulationDescriptor> enumerator = ((IDescriptorEffectProvider)entity.City).GetDescriptors().GetEnumerator())
				{
					while (enumerator.MoveNext())
					{
						if (enumerator.Current.Name == "EndlessTempleWonderInQueue")
						{
							AIParameter.AIModifier aimodifier = this.unitAssignationParameterModifier.Find((AIParameter.AIModifier modifierBoost) => modifierBoost.Name == "AIItemIndustry");
							if (aimodifier != null)
							{
								aimodifier.Value = 6f;
								break;
			...
			{
				if (this.VictoryLayer.CurrentVictoryDesign != AILayer_Victory.VictoryDesign.none)
				{
	...
	private float GenerateNewUnitDesign(UnitDesign originalUnitDesign, out UnitDesign newUnitDesign)
	{
		...
		bool flag2 = this.CurrentUnitDesign.Tags.Contains(DownloadableContent9.TagColossus);
		...
		using (InterpreterContext.InterpreterSession interpreterSession = new InterpreterContext.InterpreterSession(unit))
		{
			for (int k = 0; k < this.decisionResult.Count; k++)
			{
				...
				bool flag3 = flag && (aiitemData.ItemDefinition.SubCategory == "SubCategorySword1" || aiitemData.ItemDefinition.SubCategory == "SubCategoryAxe1" || aiitemData.ItemDefinition.SubCategory == "SubCategoryHammer1");
				int l;
				if (aiitemData != null && this.HasAtLeastOneSlotEmptyAndNoUnknowSlot(array, newUnitDesign.UnitEquipmentSet, aiitemData.ItemDefinition, unit, out l) && this.departmentOfDefense.IsEquipmentItemPrerequisitesValid(unit, aiitemData.ItemDefinition))
				{
					for (l = 0; l < aiitemData.ItemDefinition.Slots.Length; l++)
					{
						if (flag3)
						{
							...
							{
								flag4 = (flag4 || unitAbilityReference.ToString().Contains("TwinBlades"));
								if (this.SetItems.Contains(unitAbilityReference.ToString()) && (!flag4 || unitAbilityReference.ToString() != "UnitAbilityWarriorSlayer"))
								{
									goto IL_A05;
								}
							}
						...
						if (num2 >= 0)
						{
							...
							if (this.CheckAgainstResourceItemNeed(aiitemData) || flag2)
							{
								...
								for (int num7 = 0; num7 < aiitemData.ItemDefinition.Slots[l].SlotTypes.Length; num7++)
								{
									...
									if (aiunitDesignData != null)
									{
										...
										if (this.Hero == null && !aiitemData.IsNaval && !flag2)
										{
											for (int num12 = 0; num12 < aiitemData.ItemDefinition.Costs.Length; num12++)
											{
												...
												{
													Dictionary<string, float> expectedResourceUsage = this.ExpectedResourceUsage;
													string key = aiitemData.ItemDefinition.Costs[num12].ResourceName.ToString();
													expectedResourceUsage[key] -= aiitemData.ItemDefinition.Costs[num12].GetValue(base.AIEntity.Empire) * this.ResourceLimitMultiplier;
												}
											}
										}
									}
	...
	private bool CheckAgainstResourceItemNeed(AIItemData currentItemData)
	{
		...
		if (this.Governorhero && currentItemData.ToString().Contains("ItemTomeStrategic3Tier"))
		{
			AIParameter.AIModifier aimodifier = this.unitAssignationParameterModifier.Find((AIParameter.AIModifier modifierBoost) => modifierBoost.Name == "AIItemIndustry");
			if (aimodifier != null && aimodifier.Value > 2f)
			{
				for (int i = 0; i < currentItemData.ItemDefinition.Costs.Length; i++)
				{
					if (currentItemData.ItemDefinition.Costs[i].ResourceName == "Strategic3")
					{
						float num2 = -currentItemData.ItemDefinition.Costs[i].GetValue(base.AIEntity.Empire);
						return this.departmentOfTheTreasury.IsTransferOfResourcePossible(base.AIEntity.Empire, "Strategic3", ref num2);
					}
				}
			}
		}
		for (int j = 0; j < currentItemData.ItemDefinition.Costs.Length; j++)
		{
			int k = 0;
			while (k < this.currentResources.Length)
			{
				if (currentItemData.ItemDefinition.Costs[j].ResourceName == this.resourcePolicy[k].Name)
				{
					float num3;
					float num4;
					if (this.resourcePolicy[k].Name == "EmpireMoney" && num > 200f)
					{
						num3 = this.resourcePolicy[k].Value + num / 25f;
					}
					else if (((this.HeroDesign && !this.Governorhero) || (this.Governorhero && currentItemData.ItemDefinition.Slots[0].SlotType == "Accessory")) && currentItemData.ItemDefinition.Costs[j].ResourceName.ToString().Contains("Strategic") && this.departmentOfTheTreasury.TryGetResourceStockValue(base.AIEntity.Empire.SimulationObject, currentItemData.ItemDefinition.Costs[j].ResourceName, out num4, false))
					{
						num3 = this.resourcePolicy[k].Value;
						float num5 = (currentItemData.ItemDefinition.Slots[0].SlotType == "Accessory") ? 3f : 4f;
						if (num4 > num5 * currentItemData.ItemDefinition.Costs[j].GetValue(base.AIEntity.Empire) && num4 / 2f > num3)
						{
							num3 = num4 / 2f;
						}
					}
					...
					if (currentItemData.ItemDefinition.Costs[j].GetValue(base.AIEntity.Empire) + this.currentResources[k] >= num3)
					{
						return false;
					}
					break;
		...
		foreach (EvaluableMessage_RetrofitUnit evaluableMessage_RetrofitUnit2 in base.AIEntity.AIPlayer.Blackboard.GetMessages<EvaluableMessage_RetrofitUnit>(BlackboardLayerID.Empire))
		{
			...
			if (evaluableMessage_RetrofitUnit2.EvaluationState == EvaluableMessage.EvaluableMessageState.Validate && this.aiDataRepositoryHelper.TryGetAIData<AIData_Unit>(evaluableMessage_RetrofitUnit2.ElementGuid, out aidata_Unit) && aidata_Unit.Unit.UnitDesign is UnitProfile)
			{
				...
				else
				{
					...
					if (unit.Garrison is Army && this.VictoryLayer.CurrentVictoryDesign != AILayer_Victory.VictoryDesign.none)
					{
						string victorystring = "Settler";
						if (this.VictoryLayer.CurrentVictoryDesign == AILayer_Victory.VictoryDesign.Preacher)
						{
	...
	private float empireScaleLimit;
}

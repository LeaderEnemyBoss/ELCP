...
public class AIDebugMode : Amplitude.Unity.Framework.Behaviour, IAIEvaluationHelper<AIDebugMode.Element, AIDebugMode.Context>
{
	...
	private void DiplayArmyDebugger()
	{
		...
		if ((this.selectedArmy.Empire is LesserEmpire && this.aiLayerQuestManager == null) || (!(this.selectedArmy.Empire is LesserEmpire) && !(this.selectedArmy.Empire is MinorEmpire) && this.aiLayerArmyManagement == null) || (this.selectedArmy.Empire is MinorEmpire && this.aiLayerRoaming == null))
		{
			return;
		}
		if (this.selectedArmy.HasCatspaw)
		{
			AILayer_Catspaw layer = this.aiLayerArmyManagement.AIEntity.GetLayer<AILayer_Catspaw>();
			if (layer != null)
			{
				CatspawArmy catspawArmy = layer.CatspawArmies.Find((CatspawArmy c) => c.Garrison.GUID == this.selectedArmy.GUID);
				if (catspawArmy != null)
				{
					catspawArmy.DisplayDebug();
					return;
				}
			}
		}
		...
		else if (this.selectedArmy.Empire is MinorEmpire)
		{
			this.DisplayCommanders(this.aiLayerRoaming.AICommanders, (AICommander commander) => commander.Missions.Any((AICommanderMission mission) => mission.AIDataArmyGUID == this.selectedArmy.GUID));
		}
		else
		{
			...
			this.DisplayCommanders(this.aiLayerQuestSolver.AICommanders, (AICommander commander) => commander.Missions.Any((AICommanderMission mission) => mission.AIDataArmyGUID == this.selectedArmy.GUID));
		}
		UnityEngine.GUILayout.Space(10f);
	}
	...
	private void InitializeArmyDebugger(Army army)
	{
		...
		else
		{
			if (!(empire is MinorEmpire))
			{
				...
				this.aiLayerQuestSolver = entity2.GetLayer<AILayer_QuestSolver>();
				this.currentDebugMode = AIDebugMode.DebugMode.Army;
				return;
			}
			AIEntity_MinorEmpire aientity_MinorEmpire = null;
			if (!this.aiScheduler.TryGetMinorEmpireAIEntity((empire as MinorEmpire).Region, out aientity_MinorEmpire))
			{
				Diagnostics.LogError("Failed to retrieve ai player of empire {0}.", new object[]
				{
					empire.Index
			...
			this.aiDataRepository = AIScheduler.Services.GetService<IAIDataRepositoryAIHelper>();
			this.aiLayerRoaming = aientity_MinorEmpire.GetLayer<AILayer_Roaming>();
			this.currentDebugMode = AIDebugMode.DebugMode.Army;
			return;
		}
	}
	...
	private void ReleaseArmyDebugger()
	{
		...
		this.aiLayerRoaming = null;
		this.aiLayerKaiju = null;
		this.aiLayerQuestSolver = null;
		this.currentDebugMode = AIDebugMode.DebugMode.None;
	}
	...
	{
		Personality personality = this.personalityAIHelper.GetPersonality(this.playerControllerRepository.ActivePlayerController.Empire as global::Empire);
		if (personality != null)
		{
			UnityEngine.GUILayout.Label("Personalities: " + string.Join(", ", (from X in personality.Personalities
			select X.Name.ToString()).ToArray<string>()), new GUILayoutOption[0]);
		}
	...
	private void DiplayEncounterDebugger()
	{
		if (this.activePlayerAILayerEncounter != null)
		{
			...
			if (Amplitude.Unity.Framework.Application.Preferences.EnableModdingTools)
			{
				...
				if (DepartmentOfTheInterior.CanInvokePillarsAndSpells(this.activePlayerAILayerEncounter.AIEntity.Empire))
				{
					UnityEngine.GUILayout.Label(string.Format("<b>Spell Scoring</b>", new object[0]), new GUILayoutOption[0]);
					foreach (AILayer_Encounter.DebugSpellScoring debugSpellScoring in this.activePlayerAILayerEncounter.AIDebugSpellScoring)
					{
						UnityEngine.GUILayout.Label(string.Format("<i>{0}</i>: {1}, {2}", debugSpellScoring.SpellName.ToString(), debugSpellScoring.SpellScore, debugSpellScoring.SpellPosition), new GUILayoutOption[0]);
					}
				}
			}
		}
	...
	private void InitializeNavalArmyDebugger(Army army)
	{
		...
		Diagnostics.Assert(this.aiScheduler != null);
		AIPlayer_MajorEmpire aiplayer_MajorEmpire;
		if (empire is MajorEmpire && this.aiScheduler.TryGetMajorEmpireAIPlayer(empire as MajorEmpire, out aiplayer_MajorEmpire))
		{
			AIEntity_Empire entity = aiplayer_MajorEmpire.GetEntity<AIEntity_Empire>();
			if (entity == null)
			{
		...
		else
		{
			...
			AIEntity_NavalEmpire entity2 = aiplayer_NavalEmpire.GetEntity<AIEntity_NavalEmpire>();
			if (entity2 == null)
			{
				Diagnostics.LogError("Failed to retrieve ai entity empire.");
				this.ReleaseNavalArmyDebugger();
	...
	private void DisplayResearchSummary()
	{
		if (Amplitude.Unity.Framework.Application.Preferences.EnableModdingTools && this.aiLayerResearch != null)
		{
			...
			{
				UnityEngine.GUILayout.Label(string.Format("<b>Victory Focus: {0}</b>", this.aiLayerVictory.CurrentFocusEnum.ToString()), new GUILayoutOption[0]);
			}
		...
		if (this.selectedUnitDesign != null)
		{
			...
			if (this.selectedUnitDesign is UnitProfile && (this.selectedUnitDesign as UnitProfile).IsHero && this.aILayerHeroAssignation != null && Amplitude.Unity.Framework.Application.Preferences.EnableModdingTools)
			{
				...
				if (unit != null && this.aiDataRepository.TryGetAIData<AIData_Unit>(unit.GUID, out aidata_Unit))
				{
					...
					for (int j = 0; j < aidata_Unit.HeroData.LongTermSpecialtyFitness.Length; j++)
					{
						UnityEngine.GUILayout.Label(string.Format("Speciality {0}: {1} {2}", AILayer_HeroAssignation.HeroAssignationTypeNames[j], aidata_Unit.HeroData.LongTermSpecialtyFitness[j], aidata_Unit.HeroData.CurrentSpecialtyFitness[j]), new GUILayoutOption[0]);
					}
	...
	private AILayer_QuestSolver aiLayerQuestSolver;

	private IPersonalityAIHelper personalityAIHelper;

	private AILayer_Roaming aiLayerRoaming;

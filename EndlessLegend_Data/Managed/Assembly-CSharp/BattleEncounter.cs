...
public class BattleEncounter : IDisposable
{
	...
	{
		BattleEncounter.<FilterDeploymentPosition>c__AnonStorey89F <FilterDeploymentPosition>c__AnonStorey89F = new BattleEncounter.<FilterDeploymentPosition>c__AnonStorey89F();
		<FilterDeploymentPosition>c__AnonStorey89F.battleContender = battleContender;
		bool flag = false;
		Region region = this.worldPositionningService.GetRegion(<FilterDeploymentPosition>c__AnonStorey89F.battleContender.WorldPosition);
		if (region.City != null && region.City.Districts.Any((District match) => match.Type != DistrictType.Exploitation && match.WorldPosition == <FilterDeploymentPosition>c__AnonStorey89F.battleContender.WorldPosition))
		{
			flag = true;
		}
		global::Empire contenderEnemyEmpire = this.GetContenderEnemyEmpire(<FilterDeploymentPosition>c__AnonStorey89F.battleContender);
		<FilterDeploymentPosition>c__AnonStorey89F.overallPositions = new List<WorldPosition>(battleZone.GetWorldPositions());
		List<City> list = new List<City>();
		PathfindingWorldContext worldContext = new PathfindingWorldContext(battleZone, null);
		bool flag2 = <FilterDeploymentPosition>c__AnonStorey89F.battleContender.Garrison is Fortress;
		int index;
		for (index = 0; index < <FilterDeploymentPosition>c__AnonStorey89F.overallPositions.Count; index++)
		{
			if (<FilterDeploymentPosition>c__AnonStorey89F.overallPositions[index].IsValid)
			{
				bool flag3 = this.worldPositionningService.IsWaterTile(<FilterDeploymentPosition>c__AnonStorey89F.overallPositions[index]) && !this.worldPositionningService.IsFrozenWaterTile(<FilterDeploymentPosition>c__AnonStorey89F.overallPositions[index]);
				if (<FilterDeploymentPosition>c__AnonStorey89F.battleContender.Garrison is Army)
				{
					flag2 = (<FilterDeploymentPosition>c__AnonStorey89F.battleContender.Garrison as Army).IsNaval;
					if (flag2 && battleZone.CenterWorldPosition != <FilterDeploymentPosition>c__AnonStorey89F.overallPositions[index] && this.pathFindingService.FindPath(<FilterDeploymentPosition>c__AnonStorey89F.battleContender.Garrison as Army, battleZone.CenterWorldPosition, <FilterDeploymentPosition>c__AnonStorey89F.overallPositions[index], PathfindingManager.RequestMode.Default, worldContext, PathfindingFlags.IgnoreArmies | PathfindingFlags.IgnoreOtherEmpireDistrict | PathfindingFlags.IgnoreDiplomacy | PathfindingFlags.IgnoreEncounterAreas | PathfindingFlags.IgnoreFogOfWar | PathfindingFlags.IgnorePOI | PathfindingFlags.IgnoreSieges | PathfindingFlags.IgnoreKaijuGarrisons, null) == null)
					{
						battleZone.RemoveWorldPosition(<FilterDeploymentPosition>c__AnonStorey89F.overallPositions[index]);
						deploymentArea.RemoveWorldPosition(<FilterDeploymentPosition>c__AnonStorey89F.overallPositions[index]);
						goto IL_398;
					}
				}
				if (flag2 && !flag3)
				...
				else
				{
					...
					Region region2 = this.worldPositionningService.GetRegion(<FilterDeploymentPosition>c__AnonStorey89F.overallPositions[index]);
					if (region2.City != null && region2.City.Districts.Any((District match) => match.Type != DistrictType.Exploitation && match.WorldPosition == <FilterDeploymentPosition>c__AnonStorey89F.overallPositions[index]))
					{
						bool flag4 = region2.City.Empire == <FilterDeploymentPosition>c__AnonStorey89F.battleContender.Garrison.Empire;
						...
						if (region2.City.Empire != <FilterDeploymentPosition>c__AnonStorey89F.battleContender.Garrison.Empire)
						{
							deploymentArea.RemoveWorldPosition(<FilterDeploymentPosition>c__AnonStorey89F.overallPositions[index]);
						}

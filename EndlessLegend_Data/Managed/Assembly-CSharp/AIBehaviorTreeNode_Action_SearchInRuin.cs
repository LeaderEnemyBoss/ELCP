							...
							else
							{
								if (Amplitude.Unity.Framework.Application.Preferences.EnableModdingTools)
								{
									Diagnostics.Log("~~~~~ ELCP: {0}/{1} ArmyActionSearch at {2}, {3} {4} ~~~~~", new object[]
									{
										army.LocalizedName,
										army.Empire,
										(gameEntity3 as PointOfInterest).WorldPosition,
										((gameEntity3 as PointOfInterest).Interaction.Bits & army.Empire.Bits) == army.Empire.Bits,
										(gameEntity3 as PointOfInterest).UntappedDustDeposits
									});
								}

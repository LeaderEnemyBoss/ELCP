	...
	public bool IsKaijuValidForSupport(Kaiju kaiju)
	{
		...
		foreach (Unit unit in kaiju.GetActiveTroops().Units)
		{
			if (unit.UnitDesign.Tags.Contains(Kaiju.MonsterUnitTag))
			{
				...
				if (propertyValue == unit.GetPropertyValue(SimulationProperties.MaximumHealth))
				{
					return true;
				}
				if (propertyValue > 150f * (float)(this.departmentOfScience.CurrentTechnologyEraNumber - 1) + (float)(this.departmentOfScience.CurrentTechnologyEraNumber * this.departmentOfScience.CurrentTechnologyEraNumber * 10))
				{
					return true;
				}
				return false;

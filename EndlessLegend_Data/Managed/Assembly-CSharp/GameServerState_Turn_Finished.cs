...
public class GameServerState_Turn_Finished : GameServerState<GameServerState_Turn_Finished>
{
	...
	private IEnumerator WaitForAI()
	{
		...
		do
		{
			yield return null;
			flag = false;
			using (IEnumerator<BattleEncounter> enumerator = this.BattleEncounterRepositoryService.GetEnumerator())
			{
				while (enumerator.MoveNext())
				{
					if (!enumerator.Current.IsBattleFinished)
					{
						this.tickableRepository.Tick(true);
						flag = true;
						break;
					}
				}

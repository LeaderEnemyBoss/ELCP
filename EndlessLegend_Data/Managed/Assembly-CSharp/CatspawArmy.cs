...
public class CatspawArmy : ArmyWithTask
{
	...
	public override bool ValidateMainTask()
	{
		...
		{
			if (this.catspawLayer.CatspawTasks[i].CheckValidity() && this.catspawLayer.CatspawTasks[i].IsMinorArmyValid(this.army, 1f))
			{
	...
	public override void Tick()
	{
		...
		{
			if (this.State == TickableState.NoTick || this.State == TickableState.Optional)
			{
				this.ReleaseCatspaw();
			}
		}

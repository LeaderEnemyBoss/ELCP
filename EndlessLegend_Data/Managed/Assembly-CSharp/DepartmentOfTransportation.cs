...
public class DepartmentOfTransportation : Agency, IXmlSerializable, IGameStateUpdatable<GameServerState_Turn_Main>, IGameStateUpdatable<GameServerState_Turn_Finished>
{
	...
	public IFastTravelNodeGameEntity[] GetExitTravelNodesFor(Army army, Prerequisite[] prerequisites = null)
	{
		...
		{
			if (this.IsValidExitNodeFor(fastTravelNodeGameEntity, army) && (prerequisites == null || this.CheckTravelNodePrerequisites(fastTravelNodeGameEntity, prerequisites)) && this.departmentOfForeignAffairs.CanMoveOn((int)this.WorldPositionningService.GetRegionIndex(fastTravelNodeGameEntity.WorldPosition), false))
			{
				list.Add(fastTravelNodeGameEntity);
			}
		}
	...
	protected IEnumerator FastTravelProcessor(OrderFastTravel order)
	{
		...
		{
			this.ArmyPositionChange(this, new ArmyMoveEndedEventArgs(army, worldPosition, order.ValidatedDestination));
		}
		yield break;
	}
	...
	private bool GoToAndAttackPreprocessor(OrderGoToAndAttack order)
	{
		...
		PathfindingResult pathfindingResult = this.PathfindingService.FindPath(army, army.WorldPosition, order.Destination, PathfindingManager.RequestMode.Default, null, order.Flags, null);
		if (pathfindingResult == null && order.Flags != PathfindingFlags.IgnoreArmies)
		{
			pathfindingResult = this.PathfindingService.FindPath(army, army.WorldPosition, order.Destination, PathfindingManager.RequestMode.Default, null, PathfindingFlags.IgnoreArmies, null);
		}
		if (pathfindingResult != null)
		{
			order.WorldPath.Build(pathfindingResult, army.GetPropertyValue(SimulationProperties.MovementRatio), int.MaxValue, false);
	...
	protected bool MoveToPreprocessor(OrderMoveTo order)
	{
		...
		if (region != null && region.City != null && region2 != null)
		{
					...
					for (int j = 0; j < this.DepartmentOfDefense.Armies.Count; j++)
					{
						...
						if (district != null && district.City == region.City && district.Type == DistrictType.Exploitation && !this.WorldPositionningService.IsWaterTile(district.WorldPosition))
						{
							...
							while (k < region.City.Districts.Count)
							{
								...
								{
									if (this.DepartmentOfDefense.Armies[j] == army && !this.WorldPositionningService.IsWaterTile(order.To) && region2.Index == region.Index)
									{
		...
		if (army.IsNaval && region2 != null && region2.City != null && region2.City.Districts.Any((District match) => match.WorldPosition == order.To))
		{
			if (army.Units.Sum((Unit unit) => unit.GetPropertyValue(SimulationProperties.CityDefensePointLossPerTurn)) > 0f)
			{
				if (region2.City.BesiegingSeafaringArmies.Exists((Army besiegingSeafaringArmy) => besiegingSeafaringArmy.Empire.Index == army.Empire.Index))
				{
					order.IsBesiegingCity = true;
				}
				else if (region2.City.BesiegingEmpire != null && region2.City.BesiegingEmpire.Index == army.Empire.Index)
				{
					order.IsBesiegingCity = true;
				}
			}
		...
		{
			region.OnArmyLeave(army);
			region2.OnArmyEnter(army);
		}
		return true;
	}
	...
	protected IEnumerator TeleportArmyToCityProcessor(OrderTeleportArmyToCity order)
	{
		...
		City city = gameEntity as City;
		string x = "TeleportMovementCost";
		float num = city.HasProperty(x) ? city.GetPropertyValue(x) : 1f;
		if (order.ArmyActionCooldownDuration > 0f)
		{
			ArmyActionWithCooldown.ApplyCooldown(army, order.ArmyActionCooldownDuration);
		}
		foreach (Unit unit in army.Units)
		{
			float value = Mathf.Max(0f, unit.GetPropertyValue(SimulationProperties.MovementRatio) - num);
			unit.SetPropertyBaseValue(SimulationProperties.MovementRatio, value);
			unit.Refresh(false);
		}
		army.Refresh(false);
		...
		if (this.ArmyPositionChange != null)
		{
			this.ArmyPositionChange(this, new ArmyMoveEndedEventArgs(army, worldPosition, order.Destination));
		}
		yield break;
	}
